...
<details>
  ...
  class CustomDialog {
    ...
    static void show(
      ...
      MainAxisAlignment? buttonsAlignment, // optional override
  ...
  class _DialogWidgetState extends State<_DialogWidget> with SingleTickerProviderStateMixin {
    ...
    void initState() {
      ...
      // Delay to avoid dependOnInheritedWidgetOfExactType issue
    ...
    Widget _buildMedia() {
      ...
      final widgetMedia = widget.request.mediaIsGif
    ...
    Widget _buildMessage(String? msg) {
      ...
      for (final match in regex.allMatches(msg)) {
      ...
      return RichText(
        text: TextSpan(style: TextStyle(color: widget.request.messageColor, fontSize: widget.request.fontSizeMessage), children: spans),
      );
    }
        ...
        btnWidgets.add(
          Flexible(
            ...
            child: ElevatedButton(
              ...
              child: Text(b.text, style: TextStyle(color: b.textColor ?? Colors.white)),
            ),
          ),
        );
      }
    ...
    Widget _buildDialogContent() {
      ...
      switch (widget.request.mediaPosition) {
        case DialogMediaPosition.top:
          ...
          if (widget.request.title.isNotEmpty) children.add(Text(widget.request.title, style: TextStyle(fontSize: widget.request.fontSizeTitle, color: widget.request.titleColor)));
          if (widget.request.message != null) children.add(_buildMessage(widget.request.message));
          break;
                ...
                ? [
                    ...
                    Flexible(
                      child: Column(
                        ...
                        children: [
                          if (widget.request.title.isNotEmpty) Text(widget.request.title, style: TextStyle(fontSize: widget.request.fontSizeTitle, color: widget.request.titleColor)),
                          if (widget.request.message != null) _buildMessage(widget.request.message),
                        ],
                      ),
                    ),
                  ]
